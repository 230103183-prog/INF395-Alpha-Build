...
def load_ml_components():
    # app.py находится в src/, модели — в src/models/
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    MODELS_DIR = os.path.join(BASE_DIR, 'models')

    # Fallback: если запуск из корня репозитория, ищем в src/models/
    if not os.path.isdir(MODELS_DIR):
        MODELS_DIR = os.path.join(BASE_DIR, 'src', 'models')

    if not os.path.isdir(MODELS_DIR):
        raise FileNotFoundError(
            f"Папка с моделями не найдена. Ожидалось: {os.path.join(BASE_DIR, 'models')} "
            f"или {os.path.join(BASE_DIR, 'src', 'models')}"
        )
